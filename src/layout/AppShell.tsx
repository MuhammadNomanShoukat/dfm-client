...
const NAV: NavItem[] = [
  ...
  { to: '/feed', label: 'Feed', icon: GrassIcon, roles: ['super_admin', 'farm_owner', 'farm_manager'] },
  { to: '/collection', label: 'Collection', icon: LocalShippingIcon, roles: ['super_admin', 'farm_owner', 'farm_manager', 'milk_operator'] },
  { to: '/finance', label: 'Finance', icon: PaymentsIcon, roles: ['super_admin', 'farm_owner', 'farm_manager'] },
  { to: '/employees', label: 'People', icon: GroupsIcon, roles: ['super_admin', 'farm_owner'] },
  { to: '/tasks', label: 'Tasks', icon: AssignmentIcon },
  { to: '/reports', label: 'Reports', icon: AssessmentIcon, roles: ['super_admin', 'farm_owner', 'farm_manager'] },
...
export function AppShell() {
  ...
  const items = useMemo(
    () => NAV.filter((item) => !item.roles || item.roles.includes(role) || role === 'super_admin'),
    [role],
  );
  ...
  return (
    <Box sx={{ display: 'flex', minHeight: '100vh' }}>
      ...
      >
        <Toolbar>
          <IconButton aria-label="Open menu" onClick={() => setMobileOpen(true)} sx={{ mr: 2, display: { md: 'none' } }}>
            <MenuIcon />
          </IconButton>
